...
func (support *ClientSupport) ApproveClientAction(action fido_client.ClientAction, params fido_client.ClientActionRequestParams) bool {
        // AUTO-APPROVE MODE: Automatically approve all FIDO operations without user interaction
        // This is useful for headless/web-based deployments where manual terminal confirmation is not possible
        switch action {
        case fido_client.ClientActionFIDOGetAssertion:
                fmt.Printf("[AUTO-APPROVED] Login for \"%s\" with identity \"%s\"\n", params.RelyingParty, params.UserName)
                return true
        ...
        case fido_client.ClientActionU2FAuthenticate:
                fmt.Printf("[AUTO-APPROVED] Registration of U2F device\n")
                return true
        case fido_client.ClientActionU2FRegister:
                fmt.Printf("[AUTO-APPROVED] Use of U2F device\n")
                return true
        }
