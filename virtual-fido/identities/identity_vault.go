// ================================================================================
// Identity Vault - Credential Storage
// ================================================================================
//
// File: identities/identity_vault.go
// Project: Orange USB/IP Web Interface - Virtual FIDO Component
// Purpose: In-memory storage for FIDO2/U2F credential sources
//
// Credential Source Structure:
//
//      Each credential source represents a registration with a website and contains:
//      - Type: Always "public-key" for WebAuthn
//      - ID: Random 16-byte credential identifier
//      - PrivateKey: ECDSA P-256 key for signing assertions
//      - RelyingParty: Website domain (e.g., "example.com")
//      - User: User handle and display name
//      - SignatureCounter: Increments on each authentication
//
// Operations:
//   - NewIdentity(): Create credential for new registration
//   - GetMatchingCredentialSources(): Find credentials for authentication
//   - DeleteIdentity(): Remove credential by ID
//
// Persistence:
//
//      The vault is serialized via saved_state.go and encrypted using the
//      passphrase from FIDO_PASSPHRASE environment variable. Stored in vault.json.
//
// Security:
//   - Private keys never leave the vault
//   - Credential IDs are random and unique
